<?php// Your code here!$startTime = microtime(true);
$string = '<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
$count = 0;
$finalStrArray = explode(';', $string);
                                for($i=0; $i<count($finalStrArray); $i++){
    if(strpos($finalStrArray[$i], "data-position")){
        for($j=$i-1; $j>0; $j--){
            if(strpos($finalStrArray[$j], "<div")){
                break;
            }
            if(strpos($finalStrArray[$j], "top:") !== false){
            $finalStrArray[$j] = preg_replace_callback('/\b\d+/', function ($match) {
              return $match[0] + 10;
            }, $finalStrArray[$j]);
                 $count++;
                    break; 
            }
                if(strpos($finalStrArray[$j], "top:")){
                  $count++;
                $finalStrArray[$j] = preg_replace_callback('/\b\d+\b/', function ($match) {
                  return $match[0] + 10;
                }, $finalStrArray[$j]);
                $count++;
                    break;
